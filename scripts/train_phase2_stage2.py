...
def train():
    # Configuration
    BATCH_SIZE = 8
    EPOCHS = 20  # Run for 20 more epochs
    LR = 1e-4  # Lower LR for stability (was 2e-4) # STABILITY FIX
    CHECKPOINT_DIR = "checkpoints/phase2_stage2_v2"  # NEW DIR to avoid bad checkpoints
    PRETRAINED_PATH = "checkpoints/phase2_stage1/checkpoint_stage1_epoch_1.pt"
    ...
    # Initialize Model
    logging.info(f"Initializing IndraQuantum Phase 2 (Stage 2)...")
    vocab_size = 32000
    d_model = 128
    ...
    if existing_checkpoints:
        ...
        # Optional: Load optimizer state if saved, but we'll restart optimizer for now
        # as it wasn't saved in the original script
    else:
    ...
    # We allow KD but keep weights modest initially
    # STABILITY FIX: Temperature=2.0 to soften teacher logits
    criterion = SemanticDistillationLoss(
        lambda_kd=0.1, lambda_phase=0.1, lambda_mag=0.1, temperature=2.0
    ).to(device)
    # Ensure CE inside loss_v2.py is using default ignore_index=-100 (which we set)

    # Training Loop
    metrics_file = "logs/training_metrics_stage2_v2.csv"  # New log file

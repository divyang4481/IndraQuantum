...
class IndraV11Block(nn.Module):
    ...
    def forward(self, z, mask=None):
        ...
        z = residual + self.dropout(attn_out)

        residual = z
        z_norm = self.norm2(z)
        ffn_out = self.ffn(z_norm)
        z = residual + self.dropout(ffn_out)
        return z


class IndraV11(nn.Module):
    def __init__(
        self,
        vocab_size,
        d_model,
        ...
        self.layers = nn.ModuleList(
            [
                IndraV11Block(d_model, num_heads, d_ff, dropout, window_size)
